...
onmessage = function(e) {
  let [width, height, amplitude, speed, phase_local, lineWidth, now] = e.data;
  ...
  if (arr.length === 0) {
    ...
    let scalingFactor = period / 100;

    let lineWidthRatio = lineWidth / height;

    for (let x = 0; x < width; x++) {
      let y = amplitude * Math.sin(speed * (x * scalingFactor) + parseFloat(phase_local));
      // normalized and offset for lineWidth
      let draw_y = normalize(y * (1 - lineWidthRatio), 1.0, -1.0).toFixed(6)
